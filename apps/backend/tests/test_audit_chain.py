"""Testa cadeia de hash do audit-log.

Em produção rodamos em Postgres com `pytest-postgresql`. Aqui usamos
SQLite com `JSON` (em vez de `JSONB`) só para a mecânica do hash chain,
...
def test_audit_chain_grows_and_verifies(db) -> None:
...
def test_audit_pii_masked(db) -> None:
